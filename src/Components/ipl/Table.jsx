...
export default function Table() {
  ...
  const pointsTableData = Object.keys(pointsTable).length > 0 ? pointsTable : null;

  const sortedPointsTable = pointsTableData
    ? Object.entries(pointsTableData)
        .sort(([teamA], [teamB]) => {
  ...
  return (
    <div className="p-1">
      {/* <h1 className="text-center text-base font-medium m-3 text-gray-800 dark:text-gray-200">
        Points Table
      </h1> */}
        ...
        <table className="w-full table-fixed border-collapse">
          <thead>
            <tr className="text-gray-500 dark:text-gray-200 font-medium border-b border-gray-300 dark:border-gray-700">
              <th className="w-1/3 p-3 pl-5 text-left text-sm font-medium">Team</th>
          ...
          <tbody>
            {sortedPointsTable.map(([teamKey, team], index) => {
              ...
              const nrr = parseFloat(team["Net Run Rate"]) > 0 ? `+${team["Net Run Rate"]}` : team["Net Run Rate"];

              return (
                <tr key={index} className="text-black dark:text-gray-200 text-xs md:text-sm font-medium text-center border-b border-gray-300 dark:border-gray-700 ">
                  <td className="w-1/3 p-1 py-3 text-left flex gap-2 items-center">
                    {teamKey.replace("Team ", "")}
                    <img src={teamNameImageGenerator(Name).logo} alt={Name} className="w-6"/>
                    {teamNameImageGenerator(Name).shortName}
                  </td>
